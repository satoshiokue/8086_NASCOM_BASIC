...
;
;======================================================
; The updates to the original BASIC within this file ar
;
; You have permission to use this for NON COMMERCIAL US
; If you wish to use it elsewhere, please include an ac
;
; http://searle.hostei.com/grant/index.html
;
; eMail: home.micros01@btinternet.com
;
; If the above don't work, please perform an Internet s
; updated the web page hosting service.
;
;======================================================
;
; NASCOM ROM BASIC Ver 4.7, (C) 1978 Microsoft
; Scanned from source published in 80-BUS NEWS from Vol
; (May-June 1983) to Vol 3, Issue 3 (May-June 1984)
; Adapted for the freeware Zilog Macro Assembler 2.10 t
...
DIV3		EQU	WRKSPC+16H  ; <- be
DIV4		EQU	WRKSPC+1AH  ; <- inserted
SEED		EQU	WRKSPC+1CH  +5; Random number seed
LSTRND		EQU	WRKSPC+3AH  +5; Last random number
INPSUB		EQU	WRKSPC+3EH  +5; #INP (x)" Routine
INPORT		EQU	WRKSPC+3FH  +5; PORT (x)
NULLS		EQU	WRKSPC+41H  +5; Number of nulls
LWIDTH		EQU	WRKSPC+42H  +5; Terminal width
COMMAN		EQU	WRKSPC+43H  +5; Width for commas
NULFLG		EQU	WRKSPC+44H  +5; Null after input byte flag
CTLOFG		EQU	WRKSPC+45H  +5; Control "O" flag
LINESC		EQU	WRKSPC+46H  +5; Lines counter
LINESN		EQU	WRKSPC+48H  +5; Lines number
CHKSUM		EQU	WRKSPC+4AH  +5; rray load/save check sum
NMIFLG		EQU	WRKSPC+4CH  +5; Flag for NMI break routine
BRKFLG		EQU	WRKSPC+4DH  +5; Break flag
RINPUT		EQU	WRKSPC+4EH  +5; Input reflection
POINT		EQU	WRKSPC+51H  +5; "POINT" reflection (unused)
PSET		EQU	WRKSPC+54H  +5; "SET"   reflection
RESET		EQU	WRKSPC+57H  +5; "RESET" reflection
STRSPC		EQU	WRKSPC+5AH  +5; Bottom of string space
LINEAT		EQU	WRKSPC+5CH  +5; Current line number
BASTXT		EQU	WRKSPC+5EH  +5; Pointer to start of program
BUFFER		EQU	WRKSPC+61H  +5; Input buffer
STACK		EQU	WRKSPC+66H  +5; Initial stack
CURPOS		EQU	WRKSPC+0ABH +5; Character position on line
LCRFLG		EQU	WRKSPC+0ACH +5; Locate/Create flag
TYPE		EQU	WRKSPC+0ADH +5; Data type flag
DATFLG		EQU	WRKSPC+0AEH +5; Literal statement flag
LSTRAM		EQU	WRKSPC+0AFH +5; Last available RAM
TMSTPT		EQU	WRKSPC+0B1H +5; Temporary string pointer
TMSTPL		EQU	WRKSPC+0B3H +5; Temporary string pool
TMPSTR		EQU	WRKSPC+0BFH +5; Temporary string
STRBOT		EQU	WRKSPC+0C3H +5; Bottom of string space
CUROPR		EQU	WRKSPC+0C5H +5; Current operator in EVAL
LOOPST		EQU	WRKSPC+0C7H +5; First statement of loop
DATLIN		EQU	WRKSPC+0C9H +5; Line of current DATA item
FORFLG		EQU	WRKSPC+0CBH +5; "FOR" loop flag
LSTBIN		EQU	WRKSPC+0CCH +5; Last byte entered
READFG		EQU	WRKSPC+0CDH +5; Read/Input flag
BRKLIN		EQU	WRKSPC+0CEH +5; Line of break
NXTOPR		EQU	WRKSPC+0D0H +5; Next operator in EVAL
ERRLIN		EQU	WRKSPC+0D2H +5; Line of error
CONTAD		EQU	WRKSPC+0D4H +5; Where to CONTinue
PROGND		EQU	WRKSPC+0D6H +5; End of program
VAREND		EQU	WRKSPC+0D8H +5; End of variables
ARREND		EQU	WRKSPC+0DAH +5; End of arrays
NXTDAT		EQU	WRKSPC+0DCH +5; Next data item
FNRGNM		EQU	WRKSPC+0DEH +5; Name of FN argument
FNARG		EQU	WRKSPC+0E0H +5; FN argument value
FPREG		EQU	WRKSPC+0E4H +5; Floating point register
FPEXP		EQU	FPREG+3       ; Floating point exponent
SGNRES		EQU	WRKSPC+0E8H +5; Sign of result
PBUFF		EQU	WRKSPC+0E9H +5; Number print buffer
MULVAL		EQU	WRKSPC+0F6H +5; Multiplier
PROGST		EQU	WRKSPC+0F9H +5; Start of program text area
STLOOK		EQU	WRKSPC+15DH +5; Start of memory test
;
...
;
INIT:
	MOV	DX,INITAB		; Initialise workspace
	MOV	CH,INITBE-INITAB+3	; Bytes to copy
	MOV	BX,WRKSPC		; Into workspace RAM
COPY:
	XCHG	BX,DX
	MOV	AL,[BX]			; Get source
	XCHG	BX,DX
	MOV	[BX],AL			; To destination
;	LAHF
...
INITAB:
	...
	JMP	FCERR			; "USR (X)" jump (Set to Error)
	OUT	0,AL			; "OUT p,n" skeleton
	RET
	SUB	AL,0			; Division support routine
	MOV	BL,AL
	MOV	AL,BH
	SBB	AL,0
	MOV	BH,AL
	...
	MOV	AL,0
	RET
	DB	0,0,0			; Random number seed ta
	...
	DB	052H,0C7H,04FH,080H	; Last random n
	IN	AL,0			; INP (x) skeleton
	RET
	DB	1			; POS (x) number (1)
	DB	255			; Terminal width (255)
	DB	28			; Width for commas (3 colums)
	...
	DB	0			; Break flag
	JMP	TTYLIN			; Input reflection (set to TTY)
	JMP	0000H			; POINT reflection unused
	JMP	0000H			; SET reflection
	JMP	0000H			; RESET reflection
	DW	STLOOK			; Temp string space
	DW	-2			; Current line number (old)
...
HEX:
	...
	CMP	AL,0
	JZ	HEX2			; Skip output if both h
	CALL	BYT2ASC			; Convert D to ASCII
	MOV	AL,CH
	CMP	AL,'0'
	JZ	HEX1			; Don't store high digi
...
HEX2:
	...
	MOV	AL,DH
	CMP	AL,0
	JNZ	HEX3			; If upper byte was not
	MOV	AL,CH
	CMP	AL,'0'			; If high digit of lowe
	JZ	HEX4
HEX3:
...
;	SAHF
	...
	MOV	[BX],AL			; Store zero to termina
;	LAHF
	INC	BX			; Make sure PBUFF is te
;	SAHF
	MOV	[BX],AL			; Store the double zero
	POP	CX			; Get BC back
	MOV	BX,PBUFF		; Reset to start of PBU
	JMP	STR1			; Convert the PBUFF to 
;
BYT2ASC:
	...
	ADD	AL,07H			; Bring value up to ASC
ADD30:
	ADD	AL,'0'			; And make ASCII
	MOV	CL,AL			; Save converted char t
	MOV	AL,CH			; Retrieve original val
	...
	ADD	AL,07H			; Bring it up to ASCII 
ADD301:
	ADD	AL,'0'			; And make it full ASCI
	MOV	CH,AL			; Store high order byte
	RET
;
; Convert "&Hnnnn" to FPREG
; Gets a character from (HL) checks for Hexadecimal ASC
; Char is in A, NC if char is;<=>?@ A-z, CY is set if 0
HEXTFP:
	XCHG	BX,DX			; Move code string poin
	MOV	BX,0000H		; Zero out the value
	CALL	GETHEX			; Check the number for 
	JC	HXERR			; First value wasn't he
	JMP	HEXLP1			; Convert first charact
HEXLP:
	CALL	GETHEX			; Get second and addtio
	JC	HEXIT			; Exit if not a hex cha
HEXLP1:
	ADD	BX,BX			; Rotate 4 bits to the 
	ADD	BX,BX
	ADD	BX,BX
	ADD	BX,BX
	OR	AL,BL			; Add in D0-D3 into L
	MOV	BL,AL			; Save new value
	JMP	HEXLP			; And continue until al
;
GETHEX:
;	LAHF
...
;	SAHF
	XCHG	BX,DX
	MOV	AL,[BX]
	XCHG	BX,DX			; Load character at poi
	CMP	AL,' '
...
GETHEX1:
	CMP	AL,0AH
	JC	NOSUB7			; Is already in the ran
	SUB	AL,07H			; Reduce to A-F
	CMP	AL,0AH			; Value should be $0A-$
	JNC	NOSUB7
	RET				; CY set if was :
;
NOSUB7:
	...
	CMC
	RET				; CY set if it wasn't v
;
HEXIT:
	XCHG	BX,DX			; Value into DE, Code s
	MOV	AL,DH			; Load DE into AC
	MOV	CL,DL			; For prep to
	PUSH	BX
	CALL	ACPASS			; ACPASS to set AC as i
	POP	BX
	RET
;
HXERR:
...
BIN:
	CALL	TSTNUM			; Verify it's a number
	CALL	DEINT			; Get integer -32768 to
BIN2:
	PUSH	CX			; Save contents of BC
	MOV	BX,PBUFF
	MOV	CH,17			; One higher than max c
ZEROSUP:
; Suppress leading zero
	DEC	CH			; Max 16 chars
	MOV	AL,CH
	CMP	AL,01H
	JZ	BITOUT			; Always output at leas
;	RCL	DL,1
;	RCL	DH,1
	RCL	DX,1
	JNC	ZEROSUP
...
;	LAHF
	INC	BX			; Make sure PBUFF is termi
;	SAHF
	MOV	[BX],AL			; Store the double zero
	POP	CX
	MOV	BX,PBUFF
...
; Gets a character from (HL) checks for Binary ASCII number
BINTFP:
	XCHG	BX,DX			; Move code string pointer
	MOV	BX,0000H		; Zero out the value
	CALL	CHKBIN			; Check the number for binary
	JC	BINERR			; First value wasn't binary
BINIT:
	SUB	AL,'0'
	ADD	BX,BX			; Rotate HL left
	OR	AL,BL
	MOV	BL,AL
	CALL	CHKBIN			; Get second and addtio
	JNC	BINIT			; Process if a bin char
	XCHG	BX,DX			; Value into DE, Code s
	MOV	AL,DH			; Load DE into AC
	MOV	CL,DL			; For prep to
	PUSH	BX
